    ...
    return (
        <div className="drop-down-container">
            <div className="drop-down-title-container" onClick={()=>toggleOptions()}>
                <div className="drop-down-title-name">{selected ? selected.name : ''}</div>
            ...
            {optionsVisible && <ul className="drop-down-options-list">
                {options.map((option)=>(
                    <li className="drop-down-option" key={option.id} onClick={()=>handleOptionSelected(option.id)}>{option.name}</li>
                ))}
            </ul>}
        </div>)
}
